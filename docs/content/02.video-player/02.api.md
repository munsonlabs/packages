...
Loading that bundle from a CDN needs one line of setup: Vue's esm-bundler build expects its compile-time feature flags to be substituted by a bundler, and nothing does that on a CDN. Set `globalThis.__VUE_PROD_DEVTOOLS__ = false` in its own `<script>` before importing, or every element will register, appear in the DOM, and render nothing. See [Web Component Usage](https://github.com/munsonlabs/packages/tree/main/packages/video-player#web-component-usage) for a complete example. With a bundler none of this applies.

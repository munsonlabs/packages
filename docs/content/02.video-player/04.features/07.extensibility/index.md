...
| Composable                                                | Returns                                                                                                                                                                                                                                                                                                                          |
| --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `useScrubber(playerRef)`                                  | `{ scrubbing, displayPercent, previewSeconds, onInput, onChange, onTouchStart, onTouchEnd, onPointerCancel }` - the full pause-on-drag, preview, commit-on-release dance. Bind every handler: without `onTouchStart` a tap after a drag is swallowed, and without `onPointerCancel` an interrupted drag leaves the preview stuck |
...
| `useSpokenCues(player, cues, activeIndex, options?)`      | `{ canSpeak, speechEnabled, toggleSpeech }` - reads the active transcript cue aloud, borrowing the video's mute while it does. `options` takes `pitch` and `rate`                                                                                                                                                                |

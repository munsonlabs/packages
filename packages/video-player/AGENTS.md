...
```ts
import { VideoPlayer, VideoStage, VideoCard, VideoPlaceholder, VideoPlayerPlugin, getPlatform, registerPlatform } from '@munsonlabs/video-player'
import {
  PlayButton,
  MuteButton,
  FullscreenButton,
...
| `registerPlatform()`                                                                                                                                                            | Teaches the player a new platform (URL matcher + embed adapter or source resolver) without forking — see README's "Adding a custom platform"; `registerMatcher`/`registerEmbedAdapter`/`registerSourceResolver` are also exported individually |
...
- **`sources/`** — Brightcove and JW Player aren't separate adapters; `adapters/index.ts`'s `getPlatform(url)`/`resolvePlatform(url)` matcher resolves their URLs down to a native or embed source first.

import { register, sigil } from '@munsonlabs/sigil'
 ...
 * The `mlv-` prefix keeps these out of the way of a host page's own icon names: a publisher who pins
 * `mlv-play` changes the player's play button and nothing else on their page.
 */
export const ICONS: Record<string, string> = {
  'mlv-back': '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>',
  'mlv-captions':
  ...
  'mlv-expand': '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M4 12l1.41 1.41L11 7.83V20h2V7.83l5.59 5.58L20 12l-8-8-8 8z"/></svg>',
  'mlv-fullscreen-enter':
  ...
  'mlv-loop': '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z"/></svg>',
  'mlv-more':
    '<svg viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="19" cy="12" r="2"/></svg>',
  'mlv-pause': '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>',
  'mlv-pip':
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><rect x="12" y="11" width="8" height="6" rx="1" fill="currentColor" stroke="none"/></svg>',
  'mlv-play': '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>',
  'mlv-quality':
  ...
  'mlv-skip-next': '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>',
  'mlv-volume-mute':
 ...
 * loaded - that is the wholesale way to reskin the player, and `override('mlv-play', …)` the per-icon
 * way. Should that library be unregistered again, the built-ins are put back, so `unregister('mlv')`
 * means "back to the player's own icons" rather than blank controls.
 */
export function registerIcons(): void {
  if (!watching) {
    watching = true
    sigil.subscribe(registerIcons)
  }

  if (sigil.libraries.includes('mlv')) {
    return
  }

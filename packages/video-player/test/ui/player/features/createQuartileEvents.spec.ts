...
describe('checkQuartiles', () => {
  it('does nothing when total is 0', () => {
    const fire = vi.fn()
    const { checkQuartiles } = createQuartileEvents(ref(50), ref(0), fire)
  ...
  it('fires each milestone once as playback crosses it', () => {
    ...
    const current = ref(0)
    const total = ref(100)
    const { checkQuartiles } = createQuartileEvents(current, total, fire)
  ...
  it('fires every milestone already passed when checked for the first time past them all', () => {
    const fire = vi.fn()
    const { checkQuartiles } = createQuartileEvents(ref(90), ref(100), fire)
...
describe('reset', () => {
  it('lets milestones fire again after a reset', () => {
    ...
    const total = ref(100)
    const { checkQuartiles, reset } = createQuartileEvents(current, total, fire)
